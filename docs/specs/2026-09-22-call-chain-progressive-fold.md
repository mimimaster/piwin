...
5. **live 范围的截断点与豁免规则（单一折叠与叙述提拔）**：
   - **截断点只认用户面向的产出，不认过程叙述**：携带生成媒体的行（`assistantHasUserFacingGeneration`）或无工作工具的纯文本回复行（`!assistantHasWorkTools`）作为产出留在折叠外并成为截断点；中间带有叙述文字与工作工具（`assistantHasWorkTools` 为真）的行归为过程，整条链始终处于单一运行中折叠内，避免 DeepSeek 等逐步叙述的模型频繁展开收起造成剧烈跳动。
   - **末尾流式回答自然并入**：最后一行正在流式输出纯文本且未出现工具调用时留在折叠外（回答正在涌现）；一旦该行出现工具调用，则转化为过程并入折叠。
   - **折叠头提拔最新叙述**：折叠头在运行态展示最新一句叙述（`latestNarration`，取折叠内最后一条带文字过程行首行 trim 后截断至约 60 字符），呈现为 `正在运行 · <叙述> · 第 N 个工具`。窄屏下叙述优先省略号截断（CSS `text-overflow: ellipsis`），禁止换行撑高标题。
   - **完整保留既有豁免**：运行中的子代理卡（折叠止于其上一行）、已发生的错误、打开的权限门、纯思考无工具不折、整段已在 explore capsule 里不折，以及历史暂停 run 的 `isEarlierRunRow`。
