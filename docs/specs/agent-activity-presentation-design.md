...
Project Agent 的调用链仍按因果顺序保留。回合级 `已工作` disclosure 只在**这句用户 query 已经结算**之后出现：没有流式输出、没有仍在跑的工具或子 Agent、该回合不再持有 active Run，并且已经写出用户可见的正式结论，或保留可见的最终报错行。进行中不把已结束的前缀提前收进折叠组。
...
- 失败终态的例外：Host 若把同一终态错误复制到先前已完成的工作行，这些工作行可收进摘要，但最终报错行始终留在外面；先前有独立错误或真正回复时仍保留原始顺序；
