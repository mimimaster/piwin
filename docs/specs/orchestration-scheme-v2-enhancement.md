...
| 缺口 | 影响 |
|------|------|
| 方案只有 **单一** `defaultProfileId`，无 **members[]** | 无法配置 searcher/coder/reviewer 多角色与分模型 |
...
2. **Role** = 用户可增删改；新建必须写**工作描述**；默认模板：`searcher` / `coder` / `reviewer` / `tester`（可映射现有 profile）。
...
**定位**：内置「省成本、防腐烂的 scout 编排包」——主 agent（用户选的强模型 / 高 thinking）做综合与实现决策；子 agent 默认是 **searcher 探子**（便宜、low、readonly）。
...
|------|--------------------------------------|------|----------|
| `searcher` | 大范围搜索、读库、收集路径与引用；只回高密度证据，不改代码、不做最终设计 | profile `explorer` | readonly；thinking ≤ low；**鼓励用户钉便宜 chat 模型** |
| （可选内置，默认关闭或注释）`reviewer` | 只读审查实现风险 | `reviewer` | readonly；thinking ≤ low/medium |

v2 **Ultra 最小有效集 = 1 个 searcher 成员 + 强主纪律**（对齐文章「只有一种子代理模板 + 泛型」）。  
额外 role 用户可在 clone/编辑后加；**用户自建方案**才以多 role 编队为主叙事。

**内置主纪律（mainDiscipline）必须覆盖文章要点**（可中英，实现用稳定英文或 i18n）：

1. 本 turn Ultra Code 激活：积极把**会污染主上下文**的调研/广搜委派给 `searcher`。  
2. 子代理是探子：事实、路径、引用；不做最终产品决策。  
3. 并行派发后，**用工具等待结果**；在齐结果前不要自己重复同一广搜/乱改。  
4. 用返回摘要继续推理；不要重做 scout 已做的广搜。  
5. 调用时指定 `role: searcher`（或方案默认 role）；**不要**自填昂贵 model/high thinking。  
...
- Settings 对 Ultra / scout 成员显示 **强提示**：「请为 searcher 指定更便宜的已配置模型，否则子代理可能与主模型同价」；  
- **不**在代码里写死第三方模型 id。
...
|------|----------------------|--------------|-----------|
| searcher | 调研与广搜，只回证据 | explorer | readonly |
...
export type OrchestrationSchemeSettings = {
  ...
  /** v2 核心：编队成员；Ultra 默认 ≥1 searcher */
...
1. 若无 `members` 或 `members.length === 0`：  
   - 合成一个 member：  
     `role = defaultProfileId`（如 `explorer`）或映射表 `explorer→searcher`，  
     `description = profile.description 或占位`，  
     `profileId = defaultProfileId`。  
2. Ultra builtin 常量改为带 `members: [searcher…]` + 加强 `systemPreamble`。  
...
[piwin-scheme-roster]
- searcher: <description> (model: provider/model or inherit; readonly; thinking≤low)
- coder: ...
...
Ultra 默认：`exposeSpawnMetadata: false`，roster 主要一个 searcher，与文章一致。
...
BUILTIN_ULTRA_CODE_SCHEME = {
  ...
  name: 'Ultra Code',
  description:
    'Built-in scout pack for high-effort main agents: cheap readonly searcher, low thinking, wait-for-scouts discipline (article-aligned).',
  source: 'builtin',
  ...
  systemPreamble: [ /* 见 §3.3 七条，写成连贯段落 */ ].join(' '),
}
```
...
| **C** | ORCH-V2-03 | Settings：方案编辑器（纪律 + 成员 CRUD + model 下拉 + Reset/Clone） | 手动：改 Ultra searcher 模型并发送生效 |
...
| V2-T2 | Ultra resolve 含 searcher + preamble 含 wait/scout |
| V2-T3 | spawn `role: searcher` 解析到 explorer + low |
