...
同一 Run 内完成一次响应后，后续工具循环等待不再次隐藏。下一次用户提交开启的新 Run 在 Host 上重新进入首响应等待（occupancy unknown），但 Desktop 在已有历史占用时保持环可见，沿用上次确认数字。仅排队但尚未执行的消息，不改变当前 Run 的显示资格。
...
| 已提交，当前 Run 尚无首次响应；空会话首轮 | 隐藏 | 等待提示由既有 Run 状态负责；不制造 0% |
| 已提交，当前 Run 尚无首次响应；已有历史占用 | 保持显示 | 环沿用上次确认占用；Host 本轮 occupancy 仍 unknown，不提升为 current |
...
空会话首轮在首响应前隐藏。已有历史占用的后续发送不再藏环。工具循环不是新用户轮，不重复闪隐。
...
- [x] 更新 ADR 0022；新增 ADR 0067（0066 已被闪卡学习轮占用）。architecture/prd/dev-plan 无占用环专节，未改。
