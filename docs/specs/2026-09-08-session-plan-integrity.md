...
日期：2026-09-08
状态：**单 Host T1–T6 已落地；plan-store 已接入同机合作进程文件锁并有独立进程持有/等待/读改写 smoke。它不是跨机器/任意外部写者的分布式锁；测试构建产物哈希尚未验收。**
...
第一层（执行计划 T1–T6）：单 Host authority 下每路径队列、锁内 RMW、atomic 写、Run 身份与持久化。它足以针对当前事故修复。

第二层（防御性文件锁）：plan-store 与 session index 复用同一包内 helper；覆盖 plan 的创建、更新、clear、恢复和隔离，并已有独立 Node 进程的持有/等待/读改写 smoke。它只适用于同机且遵守协议的合作进程，不替代 root ownership，也不宣称跨机器一致性；异常退出与维护恢复的完整合同仍需单独加固。
