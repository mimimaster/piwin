...
2026-09-20 状态闭环：发送一次回传不等于上游已理解。每个后续用户发言开始时，Host 以 silent commentary 重发本通话最近已接纳任务的权威状态；问进度/结果也走交接通道并复用原任务，不重复执行。拒绝一条新候选时只说明「这句话没有创建新任务」，不能把之前已接纳的任务说成没开始。
...
```text
① 说话面     Live 语音模型     用户口语 + 启动摘要 + Host 上下文更新 + 短回传
...
| 层 | 可以看见 | 禁止看见 |
|----|----------|----------|
| ① 说话面 | 通话、用户口语、启动续接摘要、绑定会话打字与 Run 短结果、④ 的短结论 | Agent 工具目录、diff、其它会话 |
| ② 交接 | 候选请求、同通话近期任务/短结果、绑定会话最近若干轮、Host 解析当前聊天模型 | 工具、项目文件/指令、其它会话；Live 事件和 call slot 不带 ModelRef |
...
何时交接：用户明确要求碰文件 / 工具 / 搜索 / 权限，或查询通话里没有的项目事实。已委派任务的进度/结果问题也走同一通道，由 Host 返回权威状态而不重复执行。候选文本保留请求、否定词和不确定性；不得把感叹、半句、状态问题和说话偏好改成祈使任务。意图判断放行后才生成工作 brief。
...
因此回传不是一次性事件：Host 把「匹配 Run 已终结」和「语音模型已收到」分开。后续新的用户发言开始时，Host 静默补发最近任务的 queued/running/completed/incomplete 状态及已有短结果，让说话面能从丢失回传或插话中恢复；连续重复的 `user-speaking` activity 不重复补发。该状态仅在通话内存中保留，挂断清空。
...
11. 打开有历史的会话再开 Live，问「刚才我们在做什么」→ 语音层用启动摘要回答，不出现委派卡片。
12. 通话中在绑定会话打字发任务 → 语音层收到 commentary；Run 结束后说 takeaway。
13. 改绑到另一会话 → 语音层拿到新会话摘要；已接纳 Run 不取消。
14. 已接纳任务后漏掉首次回传，再问「开始查了吗 / 结果呢」→ 回答权威状态，不说「还没开始」，不新开重复 Run。
