...
export class RunEventCorrelator {
  ...
  correlate(
    ...
    const ownedRunId = identity === undefined
      ? undefined
      : this.lookupOwnership(sessionId, identity);
    const inferredRunId =
      explicitRunId ??
      ownedRunId ??
      executionRunId;
    ...
    if (explicitRunId === inferredRunId) {
      this.rememberRunLifecycle(sessionId, event, explicitRunId);
      return { event, accepted: true };
    }

    this.rememberRunLifecycle(sessionId, event, inferredRunId);
    return {
  ...
  private rememberRunLifecycle(
    sessionId: string,
    event: AgentEvent,
    runId: string | undefined,
  ): void {
    if (runId === undefined || event.type !== 'run/terminal') {
      return;
    }
    this.markRunTerminal(sessionId, runId);
  }
