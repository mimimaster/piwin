...
export async function handleSessionLiveCommand(
  ...
  switch (command.type) {
    ...
    case 'session/cancel-subagent': {
      ...
      if (!record) {
        return fail(requestId, 'session/cancel-subagent', `Unknown session: ${command.sessionId}`);
      }
      if (record.kind !== 'subagent') {
        return fail(requestId, 'session/cancel-subagent', 'session is not a sub-agent');
      }
    ...
    case 'session/complete-subagent': {
      ...
      if (record.kind !== 'subagent') {
        return fail(requestId, 'session/complete-subagent', 'session is not a sub-agent');
      }
      const live = context.sessions.get(command.sessionId);
      if (live) {
        try {
      ...
      if (
        ...
        try {
          const applied = await applyWorktreeToMain({
            ...
            ...(record.subagentApplyPolicy === 'explicit' && record.subagentAllowedOutputPaths
    ...
    case 'session/prompt': {
      ...
      context.runWithContext(run.runId, async () => {
        try {
          ...
          const message = error instanceof Error ? error.message : String(error);
          context.emitRunTerminal(command.sessionId, run.runId, 'failed', undefined, message);
          context.push({
            type: 'event',
            sessionId: command.sessionId,
