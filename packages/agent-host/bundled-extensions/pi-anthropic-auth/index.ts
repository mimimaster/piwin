/**
 * Anthropic Claude Pro/Max OAuth transport shaping (@gotgenes/pi-anthropic-auth).
 * @piwin-bundled-extension
 *
 * Loaded by Pi jiti as a product extension under ~/.piwin/extensions.
 * Re-registers the built-in `anthropic` provider with Claude Code request shaping
 * for OAuth tokens so interactive usage can stay on plan limits.
 */
