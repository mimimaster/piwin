/**
 * Prompts for the `code_search` search subagent.
 *
 * Provenance (checked by direct byte search against the Devin CLI 3000.2.17
 * binary, not against the fork):
 *
 * - Binary-verified verbatim: the `# IMPORTANT`, `# ENVIRONMENT`,
 *   `# THINKING RULES`, `# FAST-SEARCH DEFAULTS`, `# SOME EXAMPLES OF
 *   WORKFLOWS`, `# TOOL USE GUIDELINES` and `# ANSWER FORMAT` sections, the
 *   `[TOOL_CALLS]` example, the `SINGLE restricted_exec call` rule, the
 *   `DO NOT EVER USE MORE THAN` clause and the `%d` budget wording.
 * - **Fork-only wording** the binary does not contain: `# NO RESULTS POLICY`
 *   and `# RESULT COUNT`. Their *behavior* is corroborated by Devin data
 *   (0-file results occur in recorded sessions; the binary carries the
 *   empty-result sentence), so they are kept, but the wording is not Devin's.
 * - Devin's repo-map wrapper is unknown (`Repo Map`, `tree -L` and the text
 *   fence are all absent from the binary), so the user message uses piwin's
 *   own header.
 *
 * Placeholders: the binary formats budgets with `%d`; the template keeps
 * `{max_turns}` / `{max_commands}` / `{max_results}` and substitutes them.
 */

export type CodeSearchPromptBudget = {
  maxTurns: number;
  maxCommands: number;
  maxResults: number;
};

/** Verbatim system prompt for the search subagent loop. */
export function buildCodeSearchSystemPrompt(input: CodeSearchPromptBudget): string {
  return SYSTEM_PROMPT_TEMPLATE.replaceAll('{max_turns}', String(input.maxTurns))
    .replaceAll('{max_commands}', String(input.maxCommands))
    .replaceAll('{max_results}', String(input.maxResults));
}

/**
 * Force the final answer on the last round. Verbatim from the binary
 * (`You have no turns left` / `even if it's not complete` both match).
 */
export const CODE_SEARCH_FORCE_ANSWER =
  "You have no turns left. Now you MUST provide your final ANSWER, even if it's not complete.";

/**
 * Fixed instruction that follows the problem statement. Verbatim from the
 * binary: `Find all code in the repository relevant to this, and answer with
 * the file paths and line ranges as instructed.`
 */
export const CODE_SEARCH_ANSWER_INSTRUCTION =
  'Find all code in the repository relevant to this, and answer with the file paths and line ranges as instructed.';

/** Header for the repo-map block; piwin's own wording (Devin's is unknown). */
export const CODE_SEARCH_REPO_MAP_HEADER = 'Repo Map (tree -L {depth} /codebase):';

/**
 * Build the subagent's user message: problem statement, the fixed answer
 * instruction, then the bounded repo map.
 */
export function buildCodeSearchUserMessage(input: {
  query: string;
  repoMap: string;
  depth: number;
}): string {
  return [
    ...
    '',
    CODE_SEARCH_REPO_MAP_HEADER.replace('{depth}', String(input.depth)),
    '```text',
    input.repoMap,
    '```',
  ].join('\n');
