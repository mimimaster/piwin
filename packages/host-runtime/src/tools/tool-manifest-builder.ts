/** Compile the exact model-visible SessionToolPolicy (spec §9.3, §9.6). */

import type { SessionToolPolicy, SubagentCapability } from '@piwin/contracts';
...
/**
 * Build the exact SessionToolPolicy for one session/child. An empty capability
 * array means NO tools are allowed (spec §9.6); omitting the ceiling means
 * the global session policy applies unchanged.
 */
export function buildSessionToolPolicy(input: SessionToolPolicyInput): SessionToolPolicy {
  ...
  if (input.subagentCapabilities === undefined) {
    const customToolNames = intersectWithCeiling(input.candidate.customToolNames, policySet);
    return {
      hostTools: [],
