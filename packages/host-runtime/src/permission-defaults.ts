...
export const BUNDLED_ASK_BASH: PermissionRule[] = [
  ...
  },
  {
    target: {
      kind: 'bash',
      pattern:
        're:(?:^|[;&|])\\s*(?:tee|cp|mv|echo|cat)\\b[^\\n]*~\\/.(?:ssh\\/(?:authorized_keys|id_rsa|id_ed25519|id_ecdsa)|aws\\/credentials|aws\\/config|piwin\\/config\\.json|gitconfig|npmrc|pypirc|netrc)\\b',
    },
    decision: 'ask',
    reason: 'write-secret-file',
  },
  {
