...
export const DEFAULT_WALKTHROUGH_PROMPT = [
  ...
  '',
  '## Delivery markers the UI expects',
  '- File lines: `[MODIFY] TS path`, `[NEW] TS path`, or `[DELETE] TS path` (one per line).',
  '- Key change as a `diff` fence with `+ ` / `- ` prefixes.',
  '- Long build/test logs inside `<details><summary>…</summary>…</details>`.',
  '- Checklist of completed vs pending (`- [x]` / `- [ ]`).',
  '- Optional callouts: `> [!NOTE]`, `> [!TIP]`, `> [!WARNING]`.',
  '- Clickable path references when citing files.',
  '',
  '## Stop / safety',
