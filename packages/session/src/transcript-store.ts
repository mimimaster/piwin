...
export async function openSessionTranscriptStore(
  ...
  if (!pauseCheckpointColumns.some((column) => column.name === 'turn_policy_json')) {
    db.exec('ALTER TABLE pause_checkpoint ADD COLUMN turn_policy_json TEXT');
  }
