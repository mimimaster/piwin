...
function createStudyCoordinator(
  ...
  async function applyProjection(record: DurableStudyOperation): Promise<void> {
    if (record.targetReviewState) {
      const { itemId } = parseReviewCardId(record.targetReviewState.cardId);
      if (await itemExists(itemId)) {
        await reviewStates.write(record.targetReviewState);
      }
  ...
  async function commit(draft: OperationDraft): Promise<DurableStudyOperation> {
    ...
    try {
      ...
      const roundId = applied.targetRound?.roundId;
      if (roundId && hooks.onApplied) {
        hooks.onApplied({
          roundId,
          revision: applied.targetRound?.revision ?? applied.afterRevision,
          reason: applied.commandType,
        });
      }
