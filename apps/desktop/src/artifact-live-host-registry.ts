...
/**
 * Ordinary live-iframe budget. Non-forceKeep hosts cannot exceed this.
 * Live streaming and active Canvas (`forceKeep`) may push the count above it.
 */
...
export type ArtifactLiveHostRegistration = {
   ...
   * budget. They can push the live count above MAX_LIVE_ARTIFACT_IFRAMES.
   */
  forceKeep?: boolean;
...
/**
 * After a slot frees, promote the highest-priority waiter. forceKeep waiters
 * may exceed MAX_LIVE_ARTIFACT_IFRAMES; ordinary waiters may not.
 */
 ...
 */
export function claimArtifactLiveHost(registration: ArtifactLiveHostRegistration): {
  admitted: boolean;
} {
  const now = Date.now();
 ...
 */
export function requestArtifactLiveHost(registration: ArtifactLiveHostRegistration): {
  admitted: boolean;
} {
