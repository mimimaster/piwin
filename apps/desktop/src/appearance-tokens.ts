 ...
 *   deck-palette     authored Obsidian / Bone ramps
 ...
 */
export const PIWIN_APPEARANCE_DARK = PIWIN_APPEARANCE_OBSIDIAN;
export const PIWIN_APPEARANCE_LIGHT = PIWIN_APPEARANCE_BONE;
...
/**
 * Pre-Deck built-in ids, mapped onto their closest replacement.
 *
 * Theme selection persists in localStorage and in Host settings, so an upgrade
 * would otherwise leave existing installs pointing at an id that no longer
 * resolves. Both retired light themes collapse onto Bone.
 */
const RETIRED_THEME_IDS: Record<string, string> = {
  'piwin-dark': 'piwin-obsidian',
  'piwin-light': 'piwin-bone',
  'piwin-orange-white': 'piwin-bone',
};
...
export function isBuiltinAppearanceId(themeId: string): boolean {
  ...
  return BUILTIN_APPEARANCES.some((theme) => theme.id === id);
}

/**
 * Dark/light product faces (Obsidian / Bone and their retired ids) plus the
 * three-color Appearance overrides. Host bootstrap must not re-apply these —
...
export function resolveDesktopAppearance(theme: ThemeManifest): ThemeManifest {
  return isBuiltinAppearanceId(theme.id) ? resolveBuiltinAppearance(theme.id) : theme;
}
