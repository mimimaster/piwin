...
export function buildSlashCatalog(options: BuildSlashCatalogOptions): SlashItem[] {
  ...
  if (!conversationChat) {
    ...
    items.push({
      ...
      description: 'Set per-send scheme: /scheme off | ultra-code | reviewed-delivery | fusion',
      keywords: ['orchestration', 'ultra', 'subagent', 'scheme', 'fusion'],
      groupLabel: 'Command',
      available,
      acceptsArgs: true,
    ...
    items.push({
      ...
      name: 'fusion',
      label: 'Fusion scheme',
      description: 'Select Fusion Lead/sidekick orchestration for the next send',
      keywords: ['orchestration', 'fusion', 'sidekick', 'scheme'],
      groupLabel: 'Command',
      available,
      ...(unavailableReason ? { unavailableReason } : {}),
    });
