...
describe('ChatThread render isolation (E1)', () => {
  ...
  it('Goal strip separates Abort (cancel the run) from Leave Goal (exit the mode)', () => {
    ...
    const exitButton = container.querySelector<HTMLButtonElement>('[data-testid="goal-exit-btn"]');
  ...
  it('Goal strip reports blocked from the transcript even when the run has settled', () => {
    // The pre-P4 strip derived status from `streaming` alone, so a blocked
    // goal in a settled run read as "paused". The tool payload is the truth.
    act(() => {
      root.render(
        <PiwinUiProvider manifest={PIWIN_APPEARANCE_DARK}>
          <ChatThreadHarness
            messages={[
              { ...createUserMessage('u-goal', 'Ship it'), agentMode: 'goal' },
              {
                id: 'a-goal',
                role: 'assistant',
                text: '',
            ...
            onRetry={noop}
            onInspectSubagent={undefined}
            composerCard={{ ...composerCard, agentMode: 'goal' }}
            locale="en"
          />
        </PiwinUiProvider>,
      );
    ...
    expect(strip?.textContent).toContain('pick a strategy');
    expect(container.querySelector('[data-testid="goal-objective"]')?.textContent).toBe('Ship it');
  });
});
