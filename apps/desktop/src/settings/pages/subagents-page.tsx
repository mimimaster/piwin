...
export function SubagentProfilesPage(): ReactElement {
  ...
  const copy = isChinese
    ? {
        title: '编排方案',
        pageDescription:
          '给主代理准备一支可点名的子代理编队。你只需定义有哪些 role、各自职责（可选模型）；派谁、是否并行，由主代理自己调度。Composer 发送时选方案；选「无」则不注入。',
        noModels: '尚未配置模型。请先在「模型配置」添加 Provider。',
        schemesTitle: '方案',
        schemesDescription:
          '每个方案 = 主纪律 + 角色名单。内置 Ultra Code 是探子编队；可覆盖、克隆或自建。',
        schemeClone: '克隆',
        ...
        schemeGeneric: '泛型派发（主代理不填 model/thinking）',
        ...
        schemeId: 'ID',
        schemeDesc: '简介',
        schemeDiscipline: '主纪律（告诉主代理怎么派）',
        schemeDisciplineHint: '何时派发、等齐结果、禁止套娃等。可改。',
        schemeMembers: '子代理角色',
        schemeAddMember: '添加角色',
        schemeAddFromTemplate: '快捷添加',
        schemeRole: '角色 (role)',
        schemeRoleDesc: '职责（主代理靠这段决定何时派这个角色）',
