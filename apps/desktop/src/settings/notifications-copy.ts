...
const ZH: NotificationsCopy = {
  ...
  authorizationDescription: '由系统通知权限控制。未授权时不会出现系统横幅。',
  authorizationDescriptionManaged: '由系统通知设置管理。未授权时不会出现系统横幅。',
  statusGranted: '已开启',
  statusDenied: '已关闭',
  ...
  enabledLabel: '启用通知',
  enabledDescription: '关闭后不发送系统通知、应用内提示或应用图标提醒；角标仍由「角标」开关控制。',
  onNeedsInputLabel: '需要批准或回答时',
  onNeedsInputDescription: '权限请求或提问等待你时提醒。关闭前会再次确认。',
  onNeedsInputConfirmTitle: '关闭批准提醒？',
  onNeedsInputConfirmBody: '关闭后，任务等待批准或提问时将不再发送系统通知或应用图标提醒。',
  confirm: '关闭提醒',
  cancel: '取消',
...
const EN: NotificationsCopy = {
  ...
  authorizationDescription: 'Controlled by the system notification permission. Banners require authorization.',
  authorizationDescriptionManaged: 'Managed in the system notification settings. Banners require authorization.',
  statusGranted: 'On',
  statusDenied: 'Off',
  ...
  enabledDescription:
    'When off, no system banners, in-app toasts, or app-icon attention. Badge still follows the badge switch.',
  ...
  onNeedsInputConfirmBody:
    'You will no longer get system notifications or app-icon attention when a task is waiting for approval or an answer.',
  confirm: 'Turn off',
  cancel: 'Cancel',
  ...
  badgeLabel: 'App icon badge',
  badgeDescription: 'Show unread completed, failed, or waiting sessions on the app icon. Independent of the master switch.',
  soundLabel: 'Sound',
  soundDescription: 'Play a sound with system notifications.',
  bounceOnNeedsInputLabel: 'Alert the app icon when approval is needed',
  bounceOnNeedsInputDescription: 'Alert the app icon when the window is in the background and approval is needed.',
};
