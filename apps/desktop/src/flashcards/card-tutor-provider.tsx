...
function draftError(code: string, message: string): CardTutorError {
  return { code, message };
}
...
export function CardTutorProvider(props: {
  ...
  const startDraft = useCallback((item: FlashcardDraftSource): void => {
  ...
  const updateDraft = useCallback((edits: { front?: string; back?: string; deck?: string }): void => {
    setState((prev) => {
  ...
  const cancelDraft = useCallback((): void => {
    draftTokenRef.current += 1;
    setState((prev) => {
      if (prev.status !== 'drafting') return prev;
      ...
      setState((prev) => {
        ...
        return {
          ...
          draft: {
            ...
            saveStatus: 'idle',
            error: draftError(
              'flashcard-draft-failed',
              outcome.kind === 'validation' ? (outcome.detail ?? 'flashcard-draft-failed') : outcome.message,
            ),
            existing: null,
          },
        };
