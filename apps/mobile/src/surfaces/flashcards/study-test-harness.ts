...
export function createStudyHostFake(
  cards: StudyHarnessCard[],
  options?: { throwOnNext?: boolean },
): {
  request: (command: HostCommand) => Promise<HostResponse>;
  calls: HostCommand[];
  setHasControl: (roundId: string, hasControl: boolean) => void;
} {
  ...
  function snapshotOf(round: RoundState): FlashcardStudySnapshot {
    ...
    const snapshot: FlashcardStudySnapshot = {
      round: {
        ...
        controllerIdentity: round.hasControl ? 'mobile' : 'desktop',
  ...
  function fail(command: HostCommand, error: string, code: string): HostResponse {
    return { type: 'response', command: command.type, success: false, error, problem: { code } };
  }

  return {
    calls,
    setHasControl(roundId, hasControl) {
      const round = rounds.get(roundId);
      if (round) round.hasControl = hasControl;
    },
    request: async (command) => {
      ...
      switch (command.type) {
          ...
          return ok(command, {
            tiles: [
              ...
              },
              {
                ...
                deck: '/Users/host/.piwin/flashcards',
              },
            ],
        ...
        case 'flashcards/study/get': {
          const round = rounds.get(command.roundId);
          if (!round) return fail(command, 'round not found', 'StudyRoundNotFoundError');
          return ok(command, snapshotOf(round));
        }
        case 'flashcards/study/claim': {
          const round = requireRound(command.roundId);
          round.hasControl = true;
          round.controlEpoch += 1;
          round.revision += 1;
          return ok(command, snapshotOf(round));
        }
        ...
        case 'flashcards/study/rate': {
          if (options?.throwOnNext) throw new Error('offline');
          const round = requireRound(command.roundId);
